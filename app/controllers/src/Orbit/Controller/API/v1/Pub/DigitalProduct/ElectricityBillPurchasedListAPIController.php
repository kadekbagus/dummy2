...
class ElectricityBillPurchasedListAPIController extends PubControllerAPI
{

    public function getElectricityBillPurchasedList()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $electricityBill = PaymentTransaction::select('payment_transactions.payment_transaction_id',
                                            ...
                                            ->where('digital_products.product_type', 'electricity_bill')
                                            ->groupBy('payment_transactions.payment_transaction_id');


            $electricityBill = $electricityBill->orderBy(DB::raw("{$prefix}payment_transactions.created_at"), 'desc');

            $_electricityBill = clone $electricityBill;

            $take = PaginationNumber::parseTakeFromGet('coupon');
            $electricityBill->take($take);

            $skip = PaginationNumber::parseSkipFromGet();
            $electricityBill->skip($skip);

            $listElectricity = $electricityBill->get();
            $count = RecordCounter::create($_electricityBill)->count();

            $this->response->data = new stdClass();
            $this->response->data->total_records = $count;
            $this->response->data->returned_records = count($listElectricity);
            $this->response->data->records = $listElectricity;
