 ...
 */
 ...
 /**
  * Listener for ready to pickup order
  */
Event::listen('orbit.order.ready-for-pickup', function($orderId, $bppUserId)
{
    ...
    if ($order) {              
        ...
        $transaction = ['orderId' => $order->order_id,
                        ...
                        'followUpUrl' => Config::get('orbit.shop.gtm_url').'/my/purchases/detail?country=Indonesia&type=order&orderId='.$order->payment_transaction_id,
                        ];
...
/**
 * Listener for complete/done order
 */

Event::listen('orbit.order.complete', function($orderId, $bppUserId)
{
    if (! empty($orderId)) {
    ...
    if ($order) {              
        // bpp user that confirm order completed
        $bppUser = BppUser::select('bpp_user_id','name','email')->where('bpp_user_id', $bppUserId)->first();
        ...
        // send email to bpp user that confirm order completed
        if (isset($bppUser->email) && isset($bppUser->name)) {
            Queue::push('Orbit\\Queue\\Order\\OrderCompleteMailQueue', [
                'recipientEmail'      => $bppUser->email,
                'recipientName'       => $bppUser->name,
