    ...
    public function getSearchMerchant()
    {
        try {
            ...
            $merchants = BaseMerchant::select(
                    ...
                    'name',
                    DB::raw("(CASE
                        WHEN COUNT(pre_exports.object_id) > 0 THEN 'in_progress'
                        ELSE
                            CASE WHEN ({$prefix}media.path IS NOT NULL or {$prefix}media.path != '') and
                                    ({$prefix}base_stores.phone IS NOT NULL or {$prefix}base_stores.phone != '') and
                                    ({$prefix}base_stores.email IS NOT NULL or {$prefix}base_stores.email != '')
                                THEN 'available'
                            ELSE 'not_available'
                            END
