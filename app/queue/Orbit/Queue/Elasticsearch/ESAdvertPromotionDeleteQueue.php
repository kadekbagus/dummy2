...
/**
 * Job that handle deleting advert Promotion record for a specific promotion.
 * Don't run this job directly, because it doesn't check if the promotion is valid/available/active or not.
 * This job meant to be run after running a script that detect if promotion is not available/active anymore.
 * (or via CLI (tinker) when we are doing maintenance/must remove them anyway)
 *
 * @author Budi <budi@dominopos.com>
 */
class ESAdvertPromotionDeleteQueue
{
    /**
    ...
    public function fire($job, $data)
    {
        ...
        try {
            ...
            $message = sprintf('[Job ID: `%s`] Elasticsearch Advert Promotion Delete Index; Status: OK; ES Index Name: %s; ES Index Type: %s; Promotion ID: %s; Promotion Name: %s',
                                $job->getJobId(),
                                $esConfig['indices']['advert_promotions']['index'],
                                $esConfig['indices']['advert_promotions']['type'],
                                $newsId,
                                $newsName);
            Log::info($message);
            ...
            $message = sprintf('[Job ID: `%s`] Elasticsearch Advert Promotion Delete Index; Status: FAIL; ES Index Name: %s; ES Index Type: %s; Code: %s; Message: %s',
                                $job->getJobId(),
                                $esConfig['indices']['advert_promotions']['index'],
                                $esConfig['indices']['advert_promotions']['type'],
                                $e->getCode(),
