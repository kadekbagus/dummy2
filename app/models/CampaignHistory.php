...
class CampaignHistory extends Eloquent
{
    ...
    public static function getRowCost($campaignid, $status, $action, $now)
    {
        ...
        if ($status === 'active') {
            if ($action === "add") {
                ...
                $cost->select("number_active_tenants as tenants", "(({$tablePrefix}campaign_histories.number_active_tenants * {$tablePrefix}campaign_price.base_price) + {$tablePrefix}campaign_histories.campaign_cost) AS cost");
            }
        } else {
            if ($action === "add") {
        ...
        return $cost->join('news', 'campaign_histories.campaign_id', '=', 'news.news_id')
                  ->join('campaign_price', 'campaign_price.campaign_id', '=', 'news.news_id')
                  ->where('campaign_histories.campaign_id', '=', $campaignid)
