    ...
    public function getCRMSummaryReport()
    {
        try {
            ...
            $activities = DB::select(DB::raw("select dt.comp_date as date,
                                    ...
                                    IFNULL(MAX(CASE WHEN dt.label = 'Event View (Pop Up)' THEN dt.count END), 0) AS 'event_view_(pop_up)', 
                                    IFNULL(MAX(CASE WHEN dt.label = 'Event Click' THEN dt.count END), 0) AS 'event_click', 
                                    ...
                                    IFNULL(MAX(CASE WHEN dt.label = 'View Events Tenant List' THEN dt.count END), 0) AS 'view_events_tenant_list', 
                                    ...
                                    IFNULL(MAX(CASE WHEN dt.label = 'Read Notification Lucky Draw Number Issuance' THEN dt.count END), 0) AS 'read_notification_lucky_draw_number_issuance', 
                                    IFNULL(MAX(CASE WHEN dt.label = 'View Winner Announcement Notification' THEN dt.count END), 0) AS 'view_winner_announcement_notification'
                                FROM
                                    (
