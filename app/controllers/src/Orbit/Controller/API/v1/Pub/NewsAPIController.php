...
class NewsAPIController extends ControllerAPI
{
    ...
    public function getMallPerNews()
    {
        ...
        try{
            ...
            $prefix = DB::getTablePrefix();
            $urlToCI = URL::route('ci-news-detail', array(), false) . "?id=";
            $news = NewsMerchant::select('news.begin_date as begin_date', 'news.end_date as end_date',
                                            ...
                                            DB::raw("CONCAT(IF({$prefix}merchants.object_type = 'tenant', oms.ci_domain, {$prefix}merchants.ci_domain), '{$urlToCI}', {$prefix}news_merchant.news_id) as news_url"),
