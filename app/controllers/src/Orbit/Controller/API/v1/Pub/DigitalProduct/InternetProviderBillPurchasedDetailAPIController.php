...
class InternetProviderBillPurchasedDetailAPIController extends PubControllerAPI
{

    public function getInternetProviderBillPurchasedDetail()
    {
        $httpCode = 200;
        ...
        try {
            ...
            if (! $internetProviderBill) {
                OrbitShopAPI::throwInvalidArgument('purchased detail not found');
            }

            $internetProviderBill->payment_midtrans_info = json_decode(unserialize($internetProviderBill->payment_midtrans_info));
            $internetProviderBill->activation_code = unserialize($internetProviderBill->payload);

            $internetProviderBill->getRewards();

            $this->response->data = $internetProviderBill;
