...
class ElectricityBillPurchasedDetailAPIController extends PubControllerAPI
{

    public function getElectricityBillPurchasedDetail()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $electricityBill->payment_midtrans_info = json_decode(unserialize($electricityBill->payment_midtrans_info));
            $electricityBill->activation_code = unserialize($electricityBill->payload);

            $electricityBill->getRewards();

            $this->response->data = $electricityBill;
