...
/**
 * Process queue for sending ready to pickup email
 *
...
class ReadyToPickupMailQueue
{
    ...
    public function fire($job, $data)
    {
        try {
            $mailviews = array(
                'html' => 'emails.order.admin.ready-to-pickup-order',
            );

            $this->sendEmail($mailviews, $data);

            $message = sprintf('[Job ID: `%s`] Ready to pickup mail; Status: Success;', $job->getJobId());
            Log::info($message);
