        ...
        <thead>
          <tr class="invoice-header">
            ...
            <th class="text-right" style="font-family:'Roboto', 'Arial', sans-serif;text-align:right;padding-bottom:30px;">
              <span class="text-red invoice-title" style="color:#f43d3c;font-size:28px;font-weight:bold;">{{{ trans('email-expired-payment.header.email-type', [], '', 'id') }}}</span>
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          <tr>
            <td colspan="2" class="invoice-body" style="font-family:'Roboto', 'Arial', sans-serif;padding-top:10px;padding-bottom:20px;mso-table-lspace:0pt !important;mso-table-rspace:0pt !important;">
              <p style="font-family:'Roboto', 'Arial', sans-serif;margin:0;">
                {{ trans('email-expired-payment.body.greeting', ['customerName' => $customerName], '', 'id') }}
              </p>
              <br>
              <p style="font-family:'Roboto', 'Arial', sans-serif;margin:0;">
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.transaction_id', [], '', 'id') }}}</strong> {{ $transaction['id'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.transaction_date', [], '', 'id') }}}</strong> {{ $transactionDateTime }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.customer_name', [], '', 'id') }}}</strong> {{ $customerName }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.email', [], '', 'id') }}}</strong> {{ $customerEmail }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.phone', [], '', 'id') }}}</strong> {{ $customerPhone }}
                <br>
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_name', [], '', 'id') }}}</strong> {{ $transaction['items'][0]['name'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_price', [], '', 'id') }}}</strong> {{ $transaction['items'][0]['price'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_quantity', [], '', 'id') }}}</strong> {{ $transaction['items'][0]['quantity'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.total_amount', [], '', 'id') }}}</strong> {{ $transaction['total'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.status', [], '', 'id') }}} <span style="color:#f43d3c;">{{ trans('email-expired-payment.body.transaction_labels.status_expired', [], '', 'id') }}</span></strong>
                <br>
              </p>
              ...
              <p style="text-align: center">
                  ...
                  <a href="{{{ $buyUrl }}}" class="btn-redeem" style="font-family:'Roboto', 'Arial', sans-serif;border-radius:5px;background-color:#f43d3c;color:#fff;font-weight:bold;font-size:16px;display:inline-block;padding:10px 20px;text-decoration:none;">
                    {{{ trans('email-expired-payment.body.buttons.buy_coupon', [], '', 'id') }}}
                  </a>
              </p>

              <p style="font-family:'Roboto', 'Arial', sans-serif;margin:0;">
                <br>
                {{ trans('email-expired-payment.body.regards', [], '', 'id') }}
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td colspan="2" class="invoice-body" style="font-family:'Roboto', 'Arial', sans-serif;padding-top:10px;padding-bottom:80px;mso-table-lspace:0pt !important;mso-table-rspace:0pt !important;">
              <p style="font-family:'Roboto', 'Arial', sans-serif;margin:0;">
                {{ trans('email-expired-payment.body.greeting', ['customerName' => $customerName]) }}
              </p>
              <br>
              <p style="font-family:'Roboto', 'Arial', sans-serif;margin:0;">
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.transaction_id') }}}</strong> {{ $transaction['id'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.transaction_date') }}}</strong> {{ $transactionDateTime }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.customer_name') }}}</strong> {{ $customerName }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.email') }}}</strong> {{ $customerEmail }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.phone') }}}</strong> {{ $customerPhone }}
                <br>
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_name') }}}</strong> {{ $transaction['items'][0]['name'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_price') }}}</strong> {{ $transaction['items'][0]['price'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.coupon_quantity') }}}</strong> {{ $transaction['items'][0]['quantity'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.total_amount') }}}</strong> {{ $transaction['total'] }}
                <br>
                <strong>{{{ trans('email-expired-payment.body.transaction_labels.status') }}} <span style="color:#f43d3c;">{{ trans('email-expired-payment.body.transaction_labels.status_expired') }}</span></strong>
                <br>
              </p>
