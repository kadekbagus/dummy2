<?php
/**
 * Unit test for testing API that return list of Wordpress post.
 *
 * @author Rio Astamal <rio@dominopos.com>
 */
class WordpressPostListTest extends TestCase
{
    ...
    public function test_no_json_files_should_return_empty_list()
    {
        $response = $this->call('GET', '/api/v1/pub/blog/posts/list');
        $this->assertResponseOk();

        $decoded = json_decode($response->getContent());
        $this->assertTrue(is_array($decoded->data->records));
        $this->assertTrue(empty($decoded->data->records));
        $this->assertSame('Wordpress JSON file is not found, no data returned', $decoded->message);
    }

    public function test_json_files_exists_but_invalid()
    {
        $this->skipForNonLinux();
        file_put_contents($this->cacheFile, '{ this is not valid json }');
        ...
        $this->assertTrue(is_array($decoded->data->records));
        $this->assertTrue(empty($decoded->data->records));
        $this->assertSame('Can not parse JSON file from Wordpress', $decoded->message);
    }

    public function test_json_file_valid_should_return_list_of_posts()
    {
        $this->skipForNonLinux();
        file_put_contents($this->cacheFile, $this->dataJson2());

        $response = $this->call('GET', '/api/v1/pub/blog/posts/list');
        $this->assertResponseOk();

        $decoded = json_decode($response->getContent());
        $this->assertSame(2, $decoded->data->total_records);
        $this->assertSame(2, $decoded->data->returned_records);
        $this->assertSame(2, count($decoded->data->records));
    }

    protected function skipForNonLinux()
    {
        if (strtoupper(substr(PHP_OS, 0, 5)) !== 'LINUX') {
            $this->markTestSkipped('Skip test because non-linux OS does not have /dev/shm');
        }
    }
