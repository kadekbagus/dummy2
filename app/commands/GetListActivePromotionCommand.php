...
class GetListActivePromotionCommand extends Command
{
    ...
    public function fire()
    {
        $take = 50;
        $skip = 0;
        $now = date('Y-m-d H:i:s', strtotime('+1 day'));    // no TZ calculation
        $row = 0;
    ...
    {
        return array(
            array('fields', null, InputOption::VALUE_OPTIONAL, 'List of fields separated by comma. Default to \'*\'.', '*'),
            array('separator', null, InputOption::VALUE_OPTIONAL, 'Separator used when printing value, default to \',\'.', ','),
