...
class DashboardAPIController extends ControllerAPI
{
     ...
     * @return Illuminate\Support\Facades\Response
     */
    public function getCouponIssuedVSReedemed()
    {
        try {
            $httpCode = 200;

            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.before.auth', array($this));

            // Require authentication
            $this->checkAuth();

            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.after.auth', array($this));

            // Try to check access control list, does this user allowed to
            // perform this action
            $user = $this->api->user;
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.before.authz', array($this, $user));

/*
            if (! ACL::create($user)->isAllowed('view_coupon_report')) {
                Event::fire('orbit.dashboard.getcouponissuedvsreedemed.authz.notallowed', array($this, $user));
...
*/
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.after.authz', array($this, $user));
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.after.validation', array($this, $validator));
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.access.forbidden', array($this, $e));
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.invalid.arguments', array($this, $e));
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.query.error', array($this, $e));
            ...
            Event::fire('orbit.dashboard.getcouponissuedvsreedemed.general.exception', array($this, $e));
        ...
        $output = $this->render($httpCode);
        Event::fire('orbit.dashboard.getcouponissuedvsreedemed.before.render', array($this, &$output));
