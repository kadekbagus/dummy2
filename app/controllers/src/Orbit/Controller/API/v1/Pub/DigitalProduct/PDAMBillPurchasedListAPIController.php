...
class PDAMBillPurchasedListAPIController extends PubControllerAPI
{

    public function getPDAMBillPurchasedList()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $pdamBill = PaymentTransaction::select('payment_transactions.payment_transaction_id',
                                            ...
                                            ->where('digital_products.product_type', 'pdam_bill')
                                            ->groupBy('payment_transactions.payment_transaction_id');


            $pdamBill = $pdamBill->orderBy(DB::raw("{$prefix}payment_transactions.created_at"), 'desc');

            $_pdamBill = clone $pdamBill;
            ...
            $listPdamBill = $pdamBill->get();
            $count = RecordCounter::create($_pdamBill)->count();

            $this->response->data = new stdClass();
            $this->response->data->total_records = $count;
            $this->response->data->returned_records = count($listPdamBill);
            $this->response->data->records = $listPdamBill;
