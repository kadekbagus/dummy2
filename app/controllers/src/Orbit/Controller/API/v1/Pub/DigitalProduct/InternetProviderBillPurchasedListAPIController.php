...
class InternetProviderBillPurchasedListAPIController extends PubControllerAPI
{

    public function getInternetProviderBillPurchasedList()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $internetProviderBill = PaymentTransaction::select('payment_transactions.payment_transaction_id',
                                            ...
                                            ->where('digital_products.product_type', 'internet_provider_bill')
                                            ->groupBy('payment_transactions.payment_transaction_id');


            $internetProviderBill = $internetProviderBill->orderBy(DB::raw("{$prefix}payment_transactions.created_at"), 'desc');

            $_internetProviderBill = clone $internetProviderBill;

            $take = PaginationNumber::parseTakeFromGet('coupon');
            $internetProviderBill->take($take);

            $skip = PaginationNumber::parseSkipFromGet();
            $internetProviderBill->skip($skip);

            $listinternetProviderBill = $internetProviderBill->get();
            $count = RecordCounter::create($_internetProviderBill)->count();

            $this->response->data = new stdClass();
            $this->response->data->total_records = $count;
            $this->response->data->returned_records = count($listinternetProviderBill);
            $this->response->data->records = $listinternetProviderBill;
