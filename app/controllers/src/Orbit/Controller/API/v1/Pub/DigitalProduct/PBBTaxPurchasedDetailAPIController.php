...
class PBBTaxPurchasedDetailAPIController extends PubControllerAPI
{

    public function getPBBTaxPurchasedDetail()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $pbbTax->payment_midtrans_info = json_decode(unserialize($pbbTax->payment_midtrans_info));
            $pbbTax->activation_code = unserialize($pbbTax->payload);

            $pbbTax->getRewards();

            $this->response->data = $pbbTax;
