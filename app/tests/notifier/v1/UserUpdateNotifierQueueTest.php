<?php
/**
 * Unit testing for queue notifier Orbit\Queue\Notifier\UserUpdateNotifier
 *
...
class UserUpdateNotifierQueueTest extends TestCase
{
    /**
     ...
     * @var string
     */
    protected $configPrefix = 'orbit-notifier.user-update.';
    ...
    {
        $queueNotifier = new QUserUpdateNotifier();
        $this->assertInstanceOf('Orbit\Queue\Notifier\UserUpdateNotifier', $queueNotifier);
    }
    ...
    public function testFail_EmptyConfigData()
    {
        ...
        $user = $retailer->user;
        $qNotifier = new QUserUpdateNotifier();
        ...
        $errorMessage = sprintf('[Job ID: `%s`] There is no user-update notify data found for retailer id %s.',
    ...
    public function testFail_ConfigDataExists_butDisabled()
    {
        ...
        $errorMessage = sprintf('[Job ID: `%s`] Notify user-update found for retailer id %s but it was disabled.',
                                $this->job->getJobId(), $retailer->merchant_id);
        $this->assertSame($errorMessage, $response['message']);
    }
    ...
    public function testFail_Non200OK_Status()
    {
        ...
        $qNotifier = new QUserUpdateNotifier($this->curlWrapper);
        ...
        $errorMessage = sprintf('[Job ID: `%s`] Notify user-update User ID: `%s` to Retailer: `%s` URL: `%s` -> Error. Message: %s.',
                                $this->job->getJobId(),
                                $user->user_id,
    ...
    public function testFail_MembershipNumberUpdatedNoCard()
    {
        $userdetail = Factory::create('UserDetail');
        $retailer = Factory::create('Mall');
        $user = $userdetail->user;
        ...
        $errorMessage = sprintf('[Job ID: `%s`] Notify user-update User ID: `%s` to Retailer: `%s` URL: `%s` -> Error. Message: %s.',
                                ...
                                $retailer->merchant_id,
                                $this->configSample['url'],
                                sprintf('Can not find membership card for mall or retailer: %s', $retailer->name)
        );
        $this->assertSame($errorMessage, $response['message']);
    }

    public function testOK_MembershipNumberUpdatedAutoCreatedNumber()
    {
        $membership = Factory::create('Membership');
        $userdetail = Factory::create('UserDetail');
        $retailer = $membership->mall;
        $user = $userdetail->user;
        ...
        // Check the updated user and its particular membership number
        ...
        $updatedNumber = MembershipNumber::where('user_id', $user->user_id)
                                         ->where('membership_id', $membership->membership_id)
                                         ->first();
        $this->assertSame($this->responseSample->data->membership_number, (string)$updatedNumber->membership_number);
    }
    ...
    public function testOK_MembershipNumberUpdated()
    {
        ...
        $retailer = $membership->mall;
        $user = $userdetail->user;
        $number = Factory::create('MembershipNumber', [
            'membership_number' => '',
            'membership_id' => $membership->membership_id,
            'issuer_merchant_id' => $retailer->merchant_id
        ]);
        ...
        $updatedNumber = MembershipNumber::where('user_id', $user->user_id)
                                         ->where('membership_id', $number->membership->membership_id)
                                         ->first();
        $this->assertSame($this->responseSample->data->membership_number, (string)$updatedNumber->membership_number);
    }
    ...
    public function testOK_MembershipNotUpdated_EmailNotRecognizedOnExternalSystem()
    {
        ...
        $this->assertSame('ok', $response['status']);
        $message = sprintf('[Job ID: `%s`] Notify user-update User ID: `%s` to Retailer: `%s` URL: `%s` -> Success. Message: No data returned.',
                            $this->job->getJobId(), $user->user_id, $retailer->merchant_id, $url);
        $this->assertSame($message, $response['message']);
