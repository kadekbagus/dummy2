...
BEGIN
    ...
    /*
    *	Description:
    *		This procedure intends calculate the daily cost of the campaign id given in parameters
    *		between start and end date. The daily cost boundaries (dates) are given for the campaign running period
    *		e.g. from campaign begin_date to campaign end_date. Any dates outside campaign begin_date and end_date are not given
    *		even if i_start_date and i_end_date are outside the campaign running dates.
    *		
    *	Parameters:
    *		IN i_campaign_id CHAR(16)		: The campaign_id for which the daily cost needs to be returned. Coupon, promotion and news campaign_id are handled.
    *		IN i_campaign_type VARCHAR(20)	: The campaign type should be either: 'coupon', 'promotion' or 'news'
    *		IN i_start_date DATETIME		: The start date used to filter out the result set returned
    *		IN i_end_date DATETIME			: The end date used to filter our the result set returned
    *		IN i_mall_time_zone VARCHAR(10)	: The mall time zone used in the stored proc to work out campaign activation/deactivation and number of tenants per day  
    *										  in the mall time zone as created_at in campaign_histories is stored in UTC
    *
    *	History:
    *	02/02/2016: [TTL] Initial version 
    *	03/02/2016: [TTL] Fixed time zone issue found by Isaac
    *	04/02/2016: [TTL] Outputing now the result set in a temporary table called tmp_campaign_cost_detail instead of returning a result set
...
# First, retrieve the date of the first change found in campaign_histories table
...
# Because in sequence table, the sequence starts from 1, need to remove one hour not to miss the first 
# update in campaign_histories table. Actually, the sequence in sequence table should start at 0 to avoid doing this
