...
class ElectricityPurchasedListAPIController extends PubControllerAPI
{

    public function getElectricityPurchasedList()
    {
        $httpCode = 200;
        ...
        try {
            ...
            $electricity = $electricity->orderBy(DB::raw("{$prefix}payment_transactions.created_at"), 'desc');

            $_electricity = clone $electricity;

            $take = PaginationNumber::parseTakeFromGet('coupon');
            $electricity->take($take);

            $skip = PaginationNumber::parseSkipFromGet();
            $electricity->skip($skip);

            $listElectricity = $electricity->get();
            $count = RecordCounter::create($_electricity)->count();
