...
/**
 * Process queue for sent coupon store notification
 *
 */
use Config;
use DB;
...
class CouponStoreNotificationQueue
{
    /**
    ...
    public function fire($job, $data)
    {
        ...
        if (empty($updatedcoupon)) {
            ...
            return [
                'status' => 'fail',
                'message' => sprintf('[Job ID: `%s`] Coupon Store Notification Coupon ID %s is not found or inactive .', $job->getJobId(), $couponId)
            ];
        }
        ...
        try {
            ...
            return [
                'status' => 'ok',
                'message' => sprintf('[Job ID: `%s`] Coupon Store Notification; Status: OK; Coupon ID: %s; Total Token: %s ',
                                $job->getJobId(),
                                $couponId,
            ...
            return [
                'status' => 'fail',
                'message' => sprintf('[Job ID: `%s`] Coupon Store Notification; Status: FAIL; Coupon ID: %s; Total Token: %s; Code: %s; Message: %s',
                                $job->getJobId(),
                                $couponId,
                                count($notificationToken),
                                $e->getCode(),
